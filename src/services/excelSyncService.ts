...
function convertRawRowsToClients(rows: Record<string, any>[], credRows: Record<string, any>[] = []): ClientProject[] {
  ...
  rows.forEach((row, index) => {
    ...
    const project: ClientProject = {
      ...
      company: String(row['Company'] || clientName),
      domain: String(domain),
      stagingUrl: String(row['Staging URL'] || row['Staging'] || ''),
      status: mapStatus(row['Status']),
      healthStatus: mapHealth(row['Health'] || row['Status']),
      cmsFramework: String(row['CMS / Framework'] || row['CMS'] || row['Framework'] || 'WordPress'),
      phpNodeVersion: String(row['Stack Version'] || row['PHP/Node'] || ''),
      hostingProvider: String(row['Hosting Provider'] || row['Hosting'] || 'Cloud Hosting'),
      serverIp: String(row['Server IP'] || row['IP'] || ''),
      sslStatus: mapSslStatus(row['SSL Status'] || row['SSL']),
      sslExpiryDate: String(row['SSL Expiry'] || ''),
      domainRenewalDate: String(row['Domain Renewal'] || row['Renewal Date'] || ''),
      billingFrequency,
      projectCost: costVal,
      monthlyRetainer: costVal,
      primaryContact: {
        name: String(row['Contact Name'] || row['Contact'] || 'Primary Contact'),
        email: String(row['Contact Email'] || row['Email'] || ''),
        phone: String(row['Contact Phone'] || row['Phone'] || ''),
      },
      tags: row['Tags'] ? String(row['Tags']).split(',').map((t) => t.trim()) : ['Synced'],
      notes: String(row['Notes'] || ''),
      lastBackupDate: String(row['Last Backup'] || ''),
      credentials: [],
      tasks: [],
